...
def decode_o1(url_or_token):
    ...
    if "o1=" in url_or_token:
        try:
            o1_val = url_or_token.split("o1=")[1].split("&")[0]
        except Exception:
            pass
    ...
    try:
        o1_clean = urllib.parse.unquote(o1_val).strip().replace(" ", "+")
        o1_clean = re.sub(r'[^A-Za-z0-9+/=]', '', o1_clean)
...
class DownloaderAPIHandler(BaseHTTPRequestHandler):
    ...
    def handle_user_courses(self, data):
        ...
        try:
            ...
            if http_err.code == 401:
                self.send_json(
                    {"success": False, "error": "Unauthorized (401): Access Token has expired or is invalid. Please log in again."},
                    401,
                )
            else:
    ...
    def handle_fetch_info(self, data):
        ...
        try:
            ...
            if content_id and content_id.isdigit() and not content_id.startswith("U2FsdGVkX19") and token:
                print(f"[WARNING] Numeric contentId detected ({content_id}). Direct API calls with numeric IDs return HTTP 500.")
            ...
            if not resolved_url.startswith("http://") and not resolved_url.startswith("https://"):
                self.send_json(
                    {"success": False, "error": "Access Token is required when pasting an Encrypted Content ID (U2FsdGVk...)."},
                    400,
                )
                return

            req = urllib.request.Request(resolved_url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"})
...
def run_downloader_process(url, key_bytes, iv_hex, filename, mux_format):
    ...
    try:
        ...
        # 2. Parse Quality Stream from Master Playlist & Merge ALL Query Parameters
        if "#EXT-X-STREAM-INF" in playlist_content:
            ...
            if best_uri:
                quality_url = urllib.parse.urljoin(final_url, best_uri)
                master_parsed = urllib.parse.parse_qs(urllib.parse.urlparse(final_url).query)
                child_parsed = urllib.parse.urlparse(quality_url)
                child_query = urllib.parse.parse_qs(child_parsed.query)

                # ALL Master Query Parameters Ko Child URL Mein Copy Karein
                for param, val in master_parsed.items():
                    if param not in child_query:
                        child_query[param] = val[0]

                new_query = urllib.parse.urlencode(child_query, doseq=True)
                target_stream_url = urllib.parse.urlunparse(
                    (child_parsed.scheme, child_parsed.netloc, child_parsed.path, child_parsed.params, new_query, child_parsed.fragment)
                )

        # 3. Build Downloader Command
        if os.path.exists(N_M3U8DL_PATH):
            cmd = [
                N_M3U8DL_PATH,
                target_stream_url,
                "--save-name", filename,
                "--save-dir", WORKSPACE_DIR,
                "--tmp-dir", os.path.join(WORKSPACE_DIR, "_temp"),
            ...
            if key_bytes:
                cmd.extend(["--key", f"00000000000000000000000000000000:{key_bytes.hex()}"])
        else:
            headers_str = (
                "Origin: https://web.classplusapp.com\r\n"
                "Referer: https://web.classplusapp.com/\r\n"
        ...
        with log_lock:
            download_logs.append("[CMD] Launching Downloader Engine...")
