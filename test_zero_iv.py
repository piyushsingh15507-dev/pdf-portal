...
try:
    ...
    sub_playlist_url = (sub_playlist_uri if sub_playlist_uri.startswith("http") else master_base + "/" + sub_playlist_uri) + connector + urllib.parse.urlparse(signed_url).query
    ...
    segment_query = resolved_playlist_url.split("?", 1)[1] if "?" in resolved_playlist_url else urllib.parse.urlparse(signed_url).query
    ...
    cipher = AES.new(o1_key, AES.MODE_CBC, b'\x00'*16)
    decrypted = cipher.decrypt(encrypted_segment)
    
    print("o1 Key (Hex)        :", o1_key.hex())
    print("Decrypted Head (Hex):", decrypted[:16].hex())
    if decrypted[0] == 0x47:
        print("[✓] SUCCESS! Decrypted correctly with zero IV!")
    else:
        print("[X] Failed to decrypt with zero IV.")
