...
try:
    ...
    if "?" in first_segment_uri:
        segment_url = first_segment_uri if first_segment_uri.startswith("http") else playlist_base + "/" + first_segment_uri
    else:
        segment_url = (first_segment_uri if first_segment_uri.startswith("http") else playlist_base + "/" + first_segment_uri) + "?" + query_str

    print("[2] Fetching encrypted segment...")
    req_seg = urllib.request.Request(segment_url, headers={"User-Agent": "Mozilla/5.0"})
    with urllib.request.urlopen(req_seg, context=ctx) as res:
        encrypted_data = res.read()
    
    print(f"    Encrypted segment size: {len(encrypted_data)} bytes")

    # Set Key and IV
    real_key = bytes.fromhex("fe2184d0b17163c4ad4336f32e6504a7")
    iv = bytes.fromhex("7a76582b4201015d1d407a2332164706")

    print("[3] Decrypting in Python...")
    cipher = AES.new(real_key, AES.MODE_CBC, iv)
    decrypted = cipher.decrypt(encrypted_data)
    
    print("    Decrypted Header (Hex):", decrypted[:16].hex())
    if decrypted[0] == 0x47:
        print("[✓] SUCCESS! Decrypted segment starts with 0x47!")
    else:
        print("[X] FAILED. Decrypted header is not MPEG-TS sync byte.")
